...
//
// GML Backend calls this endpoint; without a successful response (UserUuid) the launcher does not get a session.
//
// GML_AUTH_SECRET (optional): if set AND request sends x-gml-auth-secret header, it must match.
// Gml.Backend does not send custom headers by default, so we allow requests without the header.
//
...
export async function POST(req: Request) {
  ...
  try {
    // Validate secret only if both GML_AUTH_SECRET is set AND the header is sent
    // (Gml.Backend typically doesn't send x-gml-auth-secret, so we allow requests without it)
    const sentSecret = req.headers.get('x-gml-auth-secret');
    if (GML_AUTH_SECRET && sentSecret !== null && sentSecret !== undefined) {
      if (sentSecret !== GML_AUTH_SECRET) {
