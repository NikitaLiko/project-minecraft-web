...
import { MAIN_SITE_URL, SHOP_URL } from '@/lib/public-urls';
...
export default function Dashboard() {
  ...
  const [launcherDownload, setLauncherDownload] = useState({
    href: '/api/launcher/download',
    filename: 'PJM-Launcher.zip',
  });
  const launcherDownloadRef = useRef(launcherDownload);
  launcherDownloadRef.current = launcherDownload;
  ...
  useEffect(() => {
    fetch('/api/launcher/info')
      .then((r) => r.json())
      .then((j: { downloadUrl?: string; filename?: string }) => {
        if (j.downloadUrl && j.filename) {
          setLauncherDownload({ href: j.downloadUrl, filename: j.filename });
        }
      })
      .catch(() => {});
  }, []);
    ...
    const interval = setInterval(() => {
      setDownloadProgress(prev => {
        if (prev >= 100) {
          ...
          const dl = launcherDownloadRef.current;
          link.href = dl.href;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-display selection:bg-primary/20">
      ...
      <nav className="fixed top-0 w-full border-b border-primary/20 bg-background/90 backdrop-blur z-50">
        <div className="max-w-7xl mx-auto px-4 min-h-16 py-2 flex flex-wrap items-center justify-between gap-3 sm:gap-4">
          <div className="flex flex-wrap items-center gap-3 sm:gap-6 min-w-0">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
              <div className="w-2 h-8 bg-primary animate-pulse shrink-0" />
              <h1 className="text-lg sm:text-xl font-bold tracking-[0.2em] text-primary glitch-text truncate max-w-[10rem] sm:max-w-none" data-text="PROJECT: MINECRAFT">
                PROJECT: MINECRAFT
              </h1>
            </div>
            <div className="hidden sm:block h-6 w-px bg-primary/20 shrink-0" aria-hidden />
            <div className="flex items-center gap-8 sm:gap-10">
              <a
                href={`${MAIN_SITE_URL}/`}
                onMouseEnter={() => soundEngine.playHover()}
                className="inline-flex items-center gap-1.5 font-sans text-sm font-medium uppercase tracking-[0.12em] text-[#d1d1d1] hover:text-[#f0f0f0] transition-colors duration-200 bg-transparent border-0 p-0"
              >
                <span>{t.dashboard.to_main_site}</span>
                <ArrowUpRight className="w-[13px] h-[13px] shrink-0 text-[#d1d1d1] opacity-90" strokeWidth={1.75} aria-hidden />
              </a>
              <a
                href={`${SHOP_URL}/`}
                target="_blank"
